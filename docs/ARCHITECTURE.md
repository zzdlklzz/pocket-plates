...
- `sortable-ingredient-row.tsx` owns ingredient row fields and reorder controls, while the parent ingredient section owns the shared drag context, sensors, and field-array moves.
...
- Ingredient rows can be reordered while adding or editing a recipe. Drag handles support mouse, touch, and keyboard input, while adjacent move-up and move-down controls provide a single-tap alternative. React Hook Form keeps the reordered values together, and the repository persists their array positions through `recipe_ingredients.sort_order`.
