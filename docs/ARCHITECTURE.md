...
PocketPlates is a multi-user, private-first recipe Progressive Web App for students and beginner cooks. The current codebase has completed the Stage 1 private recipe library, discovery Slices 1–3, the weekly meal planner, and private grocery lists created manually, from selected recipes, or from one planned week. It has authenticated recipe list/detail/create/edit/archive/restore/permanent-delete flows, combined library discovery, optimized private device images, a URL-stable weekly meal agenda, and private grocery-list library/create/detail routes with grouped recipe requirements and explicit week refresh. The app also includes the Next.js shell, PWA manifest, TanStack Query provider, Supabase browser/server/proxy boundaries, complete auth flows, local SQL integration checks, signed-in mobile/desktop E2E coverage, and GitHub Actions workflow templates. Student-oriented discovery tags remain pending.
...
- CI/CD: application checks use GitHub Actions with Node.js 24 inside a version-matched Playwright container; Vercel Git deployments run lightweight verification before building, and Supabase migrations use a separate deployment workflow.
...
A loaded non-empty planner week exposes one **Grocery list** link to `/grocery-lists/new?source=meal-plan&week=<Monday>`. That authenticated server route performs one indexed, owner-scoped ID lookup: an active linked list redirects immediately to its existing detail route, while a week without one renders the generator. The generator refetches the owned week, reuses the planner prep-summary grouping to sum repeated entries, includes archived planned recipes, shows saved and planned servings with the exact scale label, and previews the same name-first grocery groups used by selected recipes. Its default editable title includes the compact week range and year; later refreshes never overwrite a custom rename. Creation refetches that authoritative week again and writes one atomic snapshot without mutating or invalidating the planner. If two already-open generators race, the unique index accepts one write and the other request resolves that specific conflict to the canonical list ID; unrelated constraint failures remain errors. Only an available week-linked list shows **Refresh from week** beside **Add item** on the existing detail page. Refresh resolves the source through the list ID, refetches the linked week, and calls the existing atomic replacement boundary; no caller-selected replacement week, confirmation, preview, diff, history, or background synchronization exists. A successful refresh preserves manual rows and matching check/override state, adds current requirements, and removes obsolete generated products. A detached week remains an editable snapshot with its saved compact week label and no refresh control.
...
The week-level Prep summary is derived entirely from the already-loaded week DTO and performs no data request or write. A pure feature-local aggregator groups entries by recipe, sums planned servings across days and meal types, preserves archived planned recipes, and sorts by first planned date, title, then recipe ID. The read-only dialog shows contributing weekdays, saved recipe yield, and an exact scale label: terminating ratios use trimmed decimals while repeating ratios use reduced fractions. Active recipe titles link to their recipe details for reference; archived recipes remain labelled and non-navigable, consistent with planner entries. The summary does not round batches, modify entries, or scale ingredients.
