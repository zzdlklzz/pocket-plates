...
- `sortable-ingredient-row.tsx` owns the compact ingredient summary, expanded ingredient fields, drag handle, and overflow actions, while the parent ingredient section owns the shared drag context, sensors, active row, field-array moves, and reversible removal state.
- `expandable-step-row.tsx` owns the compact step summary, expanded instruction field, and direct destructive action, while the parent step section owns the active row, field-array operations, and reversible removal state.
...
- Ingredient and step rows collapse into one-line summaries when they are not being edited, with section headings showing the current row count. Selecting a summary exposes that row's full controls, adding a row expands and focuses it, and field-level validation keeps affected rows open. Step rows expose their only secondary action as a direct red delete icon; ingredient rows retain an overflow control because it also contains accessible move actions.
- Removing a source, ingredient, or step shows an inline, screen-reader-announced Undo action. Undo restores the complete row at its original position, including whether an ingredient or step had been expanded, before the form is saved.
